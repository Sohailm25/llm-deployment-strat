...
> **Navigation** | [← 1.6 Data Quality](../01_data_pipeline/1.6_data_quality_assurance.md) | [2.2 Model Architecture →](2.2_model_architecture_selection.md)
>
> | | |
> |---|---|
...
│                                                              │
│  ┌─────────────────────────────────────────────────────┐    │
│  │ Character-level (9 chars = "tokenized")              │    │
│  │ [T,h,e, ,q,u,i,c,k, ,b,r,o,w,n, ,f,o,x,...]         │    │
│  │ 43 tokens → Very long sequences, slow training      │    │
│  └─────────────────────────────────────────────────────┘    │
│                                                              │
│  ┌─────────────────────────────────────────────────────┐    │
│  │ Word-level                                          │    │
│  │ [The, quick, brown, fox, jumps, over, the, lazy, dog]│   │
│  │ 9 tokens → Huge vocabulary, OOV problems            │    │
│  └─────────────────────────────────────────────────────┘    │
│                                                              │
│  ┌─────────────────────────────────────────────────────┐    │
│  │ Subword BPE (GPT-style)                              │    │
│  │ [The, Ġquick, Ġbrown, Ġfox, Ġjumps, Ġover, ...]    │    │
│  │ 10 tokens → Balanced vocabulary and efficiency      │    │
│  └─────────────────────────────────────────────────────┘    │
│                                                              │
│  Impact on Training:                                        │
│  • Sequence length → Memory & compute cost                  │
│  • Vocabulary size → Embedding table size                   │
│  • Token fertility → Model's view of text                   │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
def estimate_vocab_size(
    ...
    # Typical ranges:
    # 1B tokens → 10K vocab
    # 10B tokens → 32K vocab
    # 100B+ tokens → 100K+ vocab
...
tokens = sp.encode(text, out_type=str)  # ['▁Machine', '▁learning', ...]
...
| T5 | SentencePiece Unigram | 32,100 | ▁ prefix for spaces |
...
> **Navigation**
> [← 1.6 Data Quality](../01_data_pipeline/1.6_data_quality_assurance.md) | **[Index](../README.md#15-repository-structure)** | [2.2 Model Architecture →](2.2_model_architecture_selection.md)
