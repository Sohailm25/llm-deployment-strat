> **Navigation** | [← 13.3 Capacity Planning](13.3_capacity_planning_guide.md) | [13.5 Operational Runbooks →](13.5_operational_runbooks_guide.md)
>
> | | |
> |---|---|
...
Required Actions:
{chr(10).join(f'• {action}' for action in level.required_actions)}
...
| Avg Acknowledgment Time | {report['average_ack_time_seconds']/60:.1f} min | {'✅' if report['health_indicators']['ack_time_healthy'] else '⚠️'} |
| Avg Resolution Time | {report['average_resolve_time_seconds']/60:.1f} min | {'✅' if report['health_indicators']['resolve_time_healthy'] else '⚠️'} |
| Escalation Rate | {report['escalation_rate']*100:.1f}% | {'✅' if report['health_indicators']['escalation_rate_healthy'] else '⚠️'} |
| False Positive Rate | {report['false_positive_rate']*100:.1f}% | {'✅' if report['health_indicators']['false_positive_rate_healthy'] else '⚠️'} |
...
"""
        ...
        if not report['health_indicators']['ack_time_healthy']:
            summary += "- ⚠️ Acknowledgment times are above target. Review paging configurations and on-call response procedures.\n"

        if not report['health_indicators']['resolve_time_healthy']:
            summary += "- ⚠️ Resolution times are above target. Consider improving runbooks or adding more detailed diagnostic tooling.\n"

        if not report['health_indicators']['false_positive_rate_healthy']:
            summary += "- ⚠️ High false positive rate. Review alert thresholds and reduce noise.\n"

        if not report['health_indicators']['load_balanced']:
            summary += "- ⚠️ On-call load is unbalanced. Consider adjusting rotation to distribute incidents more evenly.\n"
